...
assert(app.includes('/* ROOT_SNAPSHOT_STORAGE_V1 */'),'Falta readSnapshotLocal raíz.');
...
assert(!/\bADMIN_RUT\b/.test(app),'Persistió ADMIN_RUT en app.js.');
assert(!/\bACCESS_KEY\b/.test(app),'Persistió ACCESS_KEY en app.js.');
const hydrateStart=app.indexOf('async function hydrateCloudData');
const hydrateEnd=hydrateStart>=0?app.indexOf('\n}',hydrateStart):-1;
assert(hydrateStart>=0&&hydrateEnd>hydrateStart,'No se encontró hydrateCloudData.');
...
console.log('MAESTRO STORAGE CONTRACT V1: PASS');
