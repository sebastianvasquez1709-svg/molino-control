...
function fail(m){throw new Error('[EXISTENCE LIVE SYNC V3] '+m)}
...
const model = [
"function existenceUnifiedDisplayModel(m){",
...
"  const items=orderIneItems(source.items||[]).map(x=>{const kg=Number(x?.kg)||0,neto=Number(x?.neto)||0;return {...x,kg,neto,promedio:kg?neto/kg:0,vn:Number(x?.vn)||0,kgp:Number(x?.kgp)||0}});",
...
"  const stockNeto=Number(selected?.totalNeto)||0;",
"  const stockKg=Number(selected?.totalKg)||0;",
"  const stockUnit=stockKg?stockNeto/stockKg:0;",
"  const waiting=!!(selected&&selected.quality?.sourceType==='existencia'&&!(selected.displayIne?.items?.length));",
"  const recordTitle=selected?String(selected.periodo||selected.key):'Sin registros';",
"  const recordCount=records.length;",
"  const html=[];",
"  html.push('<div class=\"privateHero\"><div><div class=\"privateEyebrow\">🔒 ADMIN · BASE DE EXISTENCIAS</div><h1>📦 Registros de existencia</h1><p>El registro seleccionado alimenta el stock. El INE del mismo período se sincroniza desde el Maestro y pantalla/impresión usan exactamente el mismo modelo.</p></div><div class=\"privateActions\"><label class=\"uploadBtn\">📥 Cargar Registro de Existencia<input id=\"existFile\" type=\"file\" accept=\".xlsx,.xlsm\" multiple></label>'+ (selected?'<button class=\"secondary\" type=\"button\" id=\"existPrint\">🖨️ Imprimir este informe</button>':'') + '</div></div>');",
"  if(recordCount){html.push('<div class=\"card\"><div class=\"sectionTitle\"><div><h3>Historial guardado</h3><div class=\"note\">'+recordCount+' registro'+(recordCount===1?'':'s')+' · el último archivo procesado queda seleccionado automáticamente.</div></div><select id=\"existSelect\">'+records.slice().reverse().map(r=>'<option value=\"'+esc(r.key)+'\" '+(r.key===selected?.key?'selected':'')+'>'+esc(r.periodo||r.key)+' · '+esc(r.fileName||'Registro')+'</option>').join('')+'</select></div><div class=\"historyGrid\">'+records.slice().reverse().map(r=>'<button type=\"button\" class=\"historyItem '+(r.key===selected?.key?'active':'')+'\" data-exist-key=\"'+esc(r.key)+'\"><strong>'+esc(r.periodo||r.key)+'</strong><span>'+esc(r.fileName||'Registro')+'</span><b>'+money(r.totalKg)+' kg</b><small>$ '+money(r.totalNeto)+' stock</small></button>').join('')+'</div></div>')}else html.push('<div class=\"status info\">📭 No hay registros de existencia almacenados.</div>');",
"  if(selected){html.push('<div class=\"kpiGrid\"><div class=\"kpi powerKpi\"><small>VALOR STOCK</small><b>$ '+money(stockNeto)+'</b><span>Total Disponible$</span></div><div class=\"kpi powerKpi\"><small>KG STOCK</small><b>'+money(stockKg)+'</b><span>Total Disponible</span></div><div class=\"kpi powerKpi\"><small>VALOR UNITARIO STOCK</small><b>$ '+dec(stockUnit)+'</b><span>Stock $ / KG</span></div><div class=\"kpi powerKpi\"><small>INE DEL PERÍODO</small><b>'+((model&&model.items?.length)?'✅ Cargado':waiting?'⏳ Sincronizando':'⚠️ Sin datos')+'</b><span>'+esc(recordTitle)+'</span></div></div><div class=\"card\"><div class=\"sectionTitle\"><div><h3>Detalle del registro + INE</h3><div class=\"note\">'+(waiting?'Buscando el INE exacto del mismo período…':'Datos sincronizados para el registro seleccionado.')+'</div></div><button class=\"danger\" type=\"button\" id=\"existDelete\">Eliminar este registro</button></div>'+renderInePautaTable(selected)+'</div>')}",
...
"  $('existFile')?.addEventListener('change',async e=>{const files=[...(e.target.files||[])];if(!files.length)return;const token=String(files.map(f=>f.name+'|'+f.size).join('||'));try{setLoading(true,'Actualizando Existencia…','Procesando el archivo y reemplazando solo el período cargado',8);let last=null;for(const f of files){last=await parseIneWorkbook(f);if(last?.quality?.sourceType!=='existencia')throw new Error(f.name+': no se reconoció como Registro de Existencia Físico-Valorizado.');await mergeExistenceRecord(last)}if(last){state.existenceSelected=last.key;state.ineSelected=last.key;}setLoading(false);renderExistencias();toast('✅ Registro cargado y seleccionado.','ok');if(last){const official=await resolveOfficialInePeriod(last.key);const cur=state.existenceRecords.find(x=>String(x.key)===String(last.key));if(cur){cur.displayIne=official||cur.displayIne||null;cur.displayIneFetchedAt=Date.now();cur.__uploadToken=token;state.existenceRecords=state.existenceRecords.map(x=>x.key===cur.key?cur:x);if(String(state.existenceSelected)===String(cur.key))renderExistencias();}}}catch(err){setLoading(false);$('content').insertAdjacentHTML('afterbegin','<div class=\"status err\"><b>❌ Error:</b> '+esc(err?.message||String(err))+'</div>')}});",
"  $('existPrint')?.addEventListener('click',()=>printIneReport(selected));",
"  $('existDelete')?.addEventListener('click',async()=>{if(!selected?.key)return;if(!confirm('¿Eliminar '+(selected.periodo||selected.key)+'?'))return;state.existenceRecords=state.existenceRecords.filter(x=>x.key!==selected.key);await persistExistenceRecords(state.existenceRecords);state.existenceBaseRecords=state.existenceBaseRecords.filter(x=>x.key!==selected.key);let db;try{db=await idb();await new Promise((resolve,reject)=>{const tx=db.transaction(STORE_EXISTENCE_BASE,'readwrite');tx.objectStore(STORE_EXISTENCE_BASE).delete(selected.key);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});}finally{try{db?.close()}catch{}}state.ineMonths=state.ineMonths.filter(x=>x.key!==selected.key);await persistIneMonths(state.ineMonths);state.existenceSelected=state.existenceRecords.at(-1)?.key||'';renderExistencias()});",
"}",""
].join('\n');
...
"  }",
"  const rows=orderIneItems(source?.items||[]);const tr=rows.map(x=>'<tr><td>'+esc(x.name)+'</td><td class=\"num\">$ '+money(x.neto)+'</td><td class=\"num\">'+money(x.kg)+'</td><td class=\"num\">$ '+avg0(x.promedio)+'</td><td class=\"num\">'+pct1(x.vn)+'</td><td class=\"num\">'+pct1(x.kgp)+'</td></tr>').join('');",
...
console.log('EXISTENCE LIVE SYNC V3: PASS');
console.log('NEWEST UPLOAD SELECTED IMMEDIATELY: PASS');
console.log('SCREEN AND PRINT USE SAME CANONICAL MODEL: PASS');
console.log('FULL EXISTENCE+INE PRINT: PASS');
console.log('STALE DISPLAY PREVENTION: PASS');
