function renderDispatches(){
  ...
  const clientToForm=(c)=>{if(!c)return;selectDispatchClient(c);$('dClientKey').value=c?.key||'';$('dDestination').value=destinationFromClient(c)||$('dDestination').value||''};
  const existingClient=editing?(clients.find(c=>String(c.key)===String(editing.clientKey||''))||findByRutLocal(editing.rut)):null;
  if(existingClient)clientToForm(existingClient);else if(editing?.rut){const c=findByRutLocal(editing.rut);if(c)clientToForm(c)}
  setupDispatchClientAutocomplete(clients);
  $('dQty').oninput=updateDispatchKgFromQty;$('dFormat').onchange=updateDispatchKgFromQty;updateDispatchKgFromQty();$('dWeatherCheck').onclick=()=>checkDispatchWeather();
  $('dispatchClear').onclick=()=>{state.dispatchEditingId='';state.dispatchDraftItems=[];renderDispatches()};
  $('dispatchCancelEdit')?.addEventListener('click',()=>{state.dispatchEditingId='';renderDispatches()});
  ...
  $('dispatchSaveBtn').onclick=async()=>{
    ...
    const rut=formatRut(client?.rut||$('dRut').value.trim()),fecha=$('dDate').value,destination=$('dDestination').value.trim(),folio=$('dFolio').value.trim(),oc=(($('dOC').value||'').trim()||'O/C PENDIENTE'),producto=$('dProduct').value.trim(),formato=$('dFormat').value,cantidad=Number($('dQty').value||0)||0,enteredKg=Number($('dKg').value||0)||0,autoWeight=dispatchFormatWeight(formato),kg=autoWeight&&cantidad?cantidad*autoWeight:enteredKg,observacion=$('dObs').value.trim();
    ...
    if(editing){
      ...
      state.dispatchEditingId='';try{saveDispatchPlans()}catch(e){console.warn(e)}renderDispatches();toast('✅ Despacho modificado correctamente.','ok');return;
    }
    ...
    }else state.dispatchPlan.push(normalizeDispatchItem({...base,id:Date.now()+'-'+Math.random().toString(36).slice(2)}));
    try{saveDispatchPlans()}catch(e){console.warn(e)}renderDispatches();toast('✅ Despacho guardado en la planilla.','ok');
  };
  $('dispatchAddDraft')?.addEventListener('click',()=>{
    const producto=$('dProduct').value.trim(),formato=$('dFormat').value,cantidad=Number($('dQty').value||0)||0,kg0=Number($('dKg').value||0)||0,obs=$('dObs').value.trim(),w=dispatchFormatWeight(formato),kg=w&&cantidad?cantidad*w:kg0,client=clients.find(c=>String(c.key)===String($('dClientKey').value||''))||findByRutLocal($('dRut').value);
  ...
  const paintPlan=()=>{const q=String($('dispatchPlanQ')?.value||'').trim().toLowerCase(),rows=savedRows.filter(r=>!q||[r.fecha,r.cliente,r.rut,r.destination,r.producto,r.formato,r.folio,r.oc,r.observacion].join(' ').toLowerCase().includes(q));$('dispatchPlanCount').innerHTML='<span>'+rows.length.toLocaleString('es-CL')+' líneas visibles</span><span class="note">Total KG: '+moneyD(rows.reduce((a,r)=>a+Number(r.kg||0),0))+' · Unidades: '+moneyD(rows.reduce((a,r)=>a+Number(r.cantidad||0),0))+'</span>';$('dispatchPlanBody').innerHTML=rows.map(r=>'<tr><td>'+escD(r.fecha||'')+'</td><td><strong>'+escD(r.cliente||'Cliente sin nombre')+'</strong><br><span class="note">'+escD(formatRut(r.rut||''))+'</span></td><td>'+escD(r.destination||'—')+'</td><td><strong>'+escD(r.producto||'—')+'</strong><br><span class="note">'+escD(r.observacion||'')+'</span></td><td>'+escD(r.formato||'')+'</td><td class="num">'+fmtD(r.cantidad)+'</td><td class="num"><strong>'+fmtD(r.kg)+'</strong></td><td>'+escD(r.folio||'—')+'</td><td>'+escD(r.oc||'O/C PENDIENTE')+'</td><td><span class="pill">Planificado</span></td><td><div class="toolbar actionBtns"><button type="button" class="secondary smallBtn" data-edit-dispatch="'+escD(r.id)+'">✏️ Modificar</button><button type="button" class="danger smallBtn" data-delete-dispatch="'+escD(r.id)+'">🗑️ Eliminar</button></div></td></tr>').join('')||'<tr><td colspan="11" class="empty">No hay despachos con este filtro.</td></tr>';$('dispatchPlanBody').querySelectorAll('[data-edit-dispatch]').forEach(b=>b.onclick=()=>window.editDispatchById(b.dataset.editDispatch));$('dispatchPlanBody').querySelectorAll('[data-delete-dispatch]').forEach(b=>b.onclick=()=>window.deleteDispatchById(b.dataset.deleteDispatch))};
  $('dispatchPlanQ').oninput=paintPlan;paintPlan();
}
