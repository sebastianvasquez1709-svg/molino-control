...
const counterFrag=fs.readFileSync(path.join(__dirname,'counter-sacogranel.jsfrag'),'utf8').trim();
function fail(msg){throw new Error('[COUNTER SACOS/GRANEL] '+msg)}
...
if(!worker.includes('// COUNTER SACOS GRANEL V1')){
  ...
  const block=String.raw`
    ...
    const counterReports=(()=>{
      ...
      const evT=find(ev,r=>counterNorm(r?.[0])==='TOTAL GENERAL'); if(evT){push('ENVASE GRANEL','ENVASE',periodOf(ev),'ENVASE = GRANEL',evT?.[3],'kg','Pivot por envase.');push('ENVASE SACOS','ENVASE',periodOf(ev),'ENVASE = SACOS/PAPEL',evT?.[4],'kg','Pivot por envase.');push('ENVASE TOTAL','ENVASE',periodOf(ev),'ENVASE = TOTAL GENERAL',evT?.[5],'kg','No asumir alcance universal.');}
    ...
    for(const [key,p] of counterPeriodsMap.entries()){
      ...
      p.formulaDifferences=p.formulaDifferences.slice(0,100);p.sgDifferences=p.sgDifferences.slice(0,100);p.unmappedCodes=[...p.unmappedCodes].sort();p.ruleCounts=[...p.ruleCounts.values()].sort((a,b)=>b.rows-a.rows);
      p.codesCount=p.codes.size;delete p.codes;counterPeriods[key]=p;
    }
...
`;
  ...
  if(!worker.includes(snap))fail('No se encontró objeto metrics.');
...
if(!app.includes('function renderSacosGranel(){')){
  ...
  if(!app.includes(anchor))fail('No se encontró ancla de render de documentos.');
  app=app.replace(anchor,counterFrag+'\n'+anchor);
}
if(!app.includes("['counter','📦 Sacos / Granel']")){
  ...
  const neu=old+",['counter','📦 Sacos / Granel']";
  if(!app.includes(old))fail('No se encontró grupo CONTROL de navegación.');
...
if(!app.includes('counter:renderSacosGranel'))app=app.replace('existencias:renderExistencias,documents:renderDocuments','existencias:renderExistencias,counter:renderSacosGranel,documents:renderDocuments');
write(appPath,app);
console.log('COUNTER SACOS/GRANEL V1: PATCHED');
