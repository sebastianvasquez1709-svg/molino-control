...
create or replace function public.n8n_claim_import_jobs(
  p_worker_id text,
...
as $$
declare
  safe_limit integer := least(greatest(coalesce(p_limit, 1), 1), 10);
begin
  perform n8n_private.assert_request();
  if p_worker_id is null or length(btrim(p_worker_id)) not between 1 and 200 then
    raise exception using message = 'invalid worker id', errcode = '22023';
  end if;
  ...
  set status = 'error',
      finished_at = now(),
      updated_at = now(),
      last_error = coalesce(j.last_error, 'retry limit reached'),
      lease_expires_at = null,
      locked_by = null
  where j.status = 'procesando'
...
begin
  perform n8n_private.assert_request();
  if jsonb_typeof(p_errors) <> 'array'
...
create or replace function public.n8n_finish_import_job(
  ...
  p_worker_id text,
  p_status text,
  p_rows_total integer default 0,
  p_rows_valid integer default 0,
  p_rows_error integer default 0,
...
begin
  perform n8n_private.assert_request();
  if p_status not in ('validado', 'rechazado', 'error') then
    raise exception using message = 'invalid terminal status', errcode = '22023';
  end if;
  if least(
       coalesce(p_rows_total, -1),
       coalesce(p_rows_valid, -1),
       coalesce(p_rows_error, -1)
     ) < 0
     or coalesce(p_rows_valid, 0) + coalesce(p_rows_error, 0)
       > coalesce(p_rows_total, -1) then
    raise exception using message = 'invalid row counters', errcode = '22023';
  end if;
  ...
  set status = p_status,
      ...
      rows_valid = p_rows_valid,
      rows_error = p_rows_error,
  ...
  where j.id = p_job_id
    and j.status = 'procesando'
    and j.locked_by = btrim(p_worker_id)
  returning j.id, j.status, j.finished_at;
end;
$$;

revoke all on function public.n8n_authorize_request() from public, authenticated;
...
revoke all on function public.n8n_claim_import_jobs(text, integer) from public, authenticated;
revoke all on function public.n8n_heartbeat_import_job(uuid, text) from public, authenticated;
revoke all on function public.n8n_stage_import_rows(uuid, text, jsonb) from public, authenticated;
revoke all on function public.n8n_append_import_errors(uuid, text, jsonb) from public, authenticated;
revoke all on function public.n8n_finish_import_job(uuid, text, text, integer, integer, integer, jsonb, text) from public, authenticated;

grant execute on function public.n8n_authorize_request() to anon;
...
grant execute on function public.n8n_claim_import_jobs(text, integer) to anon;
grant execute on function public.n8n_heartbeat_import_job(uuid, text) to anon;
grant execute on function public.n8n_stage_import_rows(uuid, text, jsonb) to anon;
grant execute on function public.n8n_append_import_errors(uuid, text, jsonb) to anon;
grant execute on function public.n8n_finish_import_job(uuid, text, text, integer, integer, integer, jsonb, text) to anon;
