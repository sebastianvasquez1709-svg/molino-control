...
async function parseXlsx(buf){const z=await unzipEntries(buf),get=async name=>z.has(name)?td.decode(new Uint8Array(await z.get(name))):'';const wb=await get('xl/workbook.xml');const rel=await get('xl/_rels/workbook.xml.rels');const relMap={};for(const m of rel.match(/<Relationship\b[^>]*>/gi)||[]){const id=attr(m,'Id'),target=attr(m,'Target');if(id)relMap[id]=target}
  ...
  for(const m of wb.match(/<sheet\b[^>]*>/gi)||[]){const name=attr(m,'name'),rid=attr(m,'r:id')||attr(m,'id');let target=relMap[rid]||'';target=target.replace(/^\.\//,'');target=target.replace(/^\/+/, '').replace(/^\.\//,'');if(!target.startsWith('xl/'))target='xl/'+target;let rows=[];
    if(target&&z.has(target)){
      ...
      if(!targets.has(name)){
        const probe=parseSheet(xml,shared,null,120);
        const probeText=probe.flat().map(v=>String(v??'')).join(' | ').toUpperCase();
        isExistencia=/REGISTRO\s+DE\s+EXISTENCIAS|TOTAL\s+DISPONIBLE|TOTAL\s+VALORIZADO|FISICO\s*-?\s*VALORIZADO/.test(probeText);
      }
...
self.onmessage = async (e) => {
  try {
    ...
    const populatedSheetCount = parsed.filter(x => Array.isArray(x.rows) && x.rows.some(r => Array.isArray(r) && r.some(v => v !== null && v !== undefined && String(v) !== ''))).length;
    const populatedCellCount = parsed.reduce((acc,x) => acc + (x.rows||[]).reduce((a,r) => a + ((r||[]).filter(v => v !== null && v !== undefined && String(v) !== '').length),0),0);
    if(populatedSheetCount===0){
      throw new Error(`El archivo está vacío: 0 celdas con datos detectadas en ${parsed.length} hoja(s). Verifica que el Excel de Registro de Existencia se haya exportado con sus filas y columnas. Código: EMPTY_WORKBOOK.`);
    }
    ...
    // AQ = AJ + AN
    // AR = AQ * U
    // D = B/C; E = B/B15; F = C/C15; D15 = B15/C15;
    // B18 = B7+B8+B9; B19 = C7+C8+C9; B20 = B18/B19.
    // IMPORTANTE: NO se selecciona una fórmula distinta por mes.
    const MAESTRO_DOC_TYPES = Object.freeze({invoice:'Factura[FT]',ticket:'Guía[ST]',receipt:'Boleta[BT]'});
    const MAESTRO_BOLETA_LOOKUP = Object.freeze({"672":672.2689075630252,"504":504.20168067226894,"269":268.9075630252101,"403":403.36134453781517,"336":336.1344537815126,"202":201.68067226890759,"185":184.87394957983193,"160":159.6638655462185,"134":134.45378151260505,"7":6.722689075630252,"245":245.3781512605042,"420":420.1680672268908,"126":126.05042016806723,"687":687.0229,"550":549.61832,"756":756.3025210084033,"538":537.8151260504202,"605":605.0420168067227,"571":571.4285714285714,"437":436.9747899159664,"218":218.4873949579832,"151":151.26050420168067,"143":142.85714285714286,"235":235.29411764705884,"2":1.680672,"252":252.10084033613447,"168":168.0672268907563,"118":117.64705882352942,"150":150.0,"137":137.0,"115":115.12605042016807,"107":107.22689075630252,"1361":1361.3445378151262,"138":137.81512605042016,"200":200.0,"220":220.16806722689077,"361":361.00840336134456,"360":360.0,"303":302.52100840336135,"471":470.5882352941177,"366":366.44,"305":305.32,"427":427.48,"13":13.445378151260504,"8403":8403.361344537816,"588":588.2352941176471,"0":0.4,"496":495.7983193277311,"140":140.0,"840":840.3361344537816,"286":285.7142857142857,"155":155.46218487394958,"188":188.23529411764707,"192":191.5966386554622,"292":292.436974789916,"190":189.91596638655463,"3":3.361344537815126,"8":8.403361344537815,"611":672.2689075630253,"739":739.4957983193277,"171":171.42857142857144,"17":16.8067226890756,"739.48":739.4952380952382,"277":277.3109243697479,"706":705.8823529411765,"641":641.22,"147":147.05882352941177,"131":131.09243697478993,"484":484.0336134453782,"130":130.25210084033614,"122":121.84873949579833,"109":109.2436974789916,"763":763.0,"84":84.0,"128":127.73109243697479,"610.67999999999995":610.68,"90":89.91596638655463,"101":100.84033613445379,"6723":6722.689075630253});
    const normalizeMovtoKey = (v) => {
      const x = Number(v);
    ...
    const deriveIneFromExistenceDetail = (detailRows, periodo) => {
      ...
      for(const r of (detailRows||[])){
        ...
        if(!kg) continue;
        if(!agg.has(fam)){
          unmapped.push({code:r?.code||'',name:r?.name||r?.item||'',docto:r?.docto||'',folio:r?.folio||'',kg,valorMovto:n(r?.valorMovto),reason:'código sin familia INE reconocida por el catálogo Maestro'});
          continue;
        }
        const z=agg.get(fam);
        ...
        z.sources.add(fx.doc===MAESTRO_DOC_TYPES.receipt?'AQ=AN(VLOOKUP CODIGOS!R16:S111)':((fx.doc===MAESTRO_DOC_TYPES.invoice||fx.doc===MAESTRO_DOC_TYPES.ticket)?'AQ=AJ(Valor Movto)':'AQ=0(Doc no contemplado)'));
      }
      const items=INE_FAMILIES.map(name=>{
        const z=agg.get(name),kg=z?.kg||0,neto=z?.neto||0;
        return {name,kg,neto,promedio:kg?neto/kg:null,vn:0,kgp:0,rows:z?.rows||0,formulaAvg:kg?neto/kg:null,sourceCodes:[...(z?.codes||[])],formulaSources:[...(z?.sources||[])]};
      });
      const totalKg=items.reduce((a,x)=>a+x.kg,0);
      const totalNeto=items.reduce((a,x)=>a+x.neto,0);
      const netoHarinas=items.slice(0,3).reduce((a,x)=>a+x.neto,0);
    ...
    // V49.0: El INE de Existencia usa una única fórmula universal; los datos mensuales del Maestro son solo fuente de auditoría.
    ...
    // V49.0: si el archivo contiene un Registro de Existencia Físico-Valorizado,
      ...
      if(rh>=0){
        ...
        if(regName){
          ...
          if(header>=0){const h=(ir[header]||[]).map(headerKey),idx={};h.forEach((v,i)=>{if(v)idx[v]=i});ine.quality={mode:'Registro de Existencia',sourceType:'existencia',headerFound:true,missing:[],range:range||'',emissionDate:emission?.replace(/^.*?:\s*/,'')||''};const summary=[],detail=[];
            ...
            const derivedIne=deriveIneFromExistenceDetail(detail,ine.periodo);
