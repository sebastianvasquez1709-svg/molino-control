...
(() => {
  ...
  function calculate(snap){
    ...
    let lastDate='';for(const d of activeDocs){const x=dateKey(d?.fecha);if(x>lastDate)lastDate=x}
  ...
  function renderError(err){
    const content=$id('content');if(!content)return;
  ...
  async function renderMacro(){
    ...
    try{
      ...
      let acc=0;const totalMix=Math.max(1,m.docMix.reduce((a,x)=>a+x[1],0));const mixStops=[];for(const [,count,color] of m.docMix){const next=acc+(count/totalMix*100);mixStops.push(`${color} ${acc}% ${next}%`);acc=next}
      ...
      content.innerHTML=`<div class="mcV2">
        ...
        <div class="mcV2Hero"><div class="mcV2HeroCard"><div class="mcV2Eyebrow">MOLINO CONTROL · VISIÓN EJECUTIVA</div><h4>$ ${money(m.salesNeto)}</h4><p>Ventas netas consolidadas de Facturas y Boletas, ajustadas por Notas de Crédito y Débito. Las Guías quedan separadas del indicador comercial.</p><div class="mcV2HeroMetric"><div><span>KG vendidos</span><strong>${qty(m.kilos)} kg</strong></div><div><span>Valor promedio</span><strong>$ ${money(avgKg)}/kg</strong></div><div><span>Último período</span><strong>${esc(m.lastDate?monthLabel(m.lastDate.slice(0,7)):'Sin fecha')}</strong></div></div></div><div class="mcV2HeroCard mcV2Health"><h4>Control del sistema</h4><div class="mcHealthItem"><span>Documentos</span><b>${m.docs.length.toLocaleString('es-CL')}</b></div><div class="mcHealthItem"><span>Clientes</span><b>${m.clients.length.toLocaleString('es-CL')}</b></div><div class="mcHealthItem"><span>Productos</span><b>${m.products.length.toLocaleString('es-CL')}</b></div><div class="mcHealthItem"><span>Despachos</span><b>${m.dispatches.length.toLocaleString('es-CL')}</b></div><div class="mcHealthItem"><span>KG despachados</span><b>${qty(m.dispatchKg)}</b></div></div></div>
        <div class="mcV2Kpis"><div class="mcKpi mcAccentBlue"><small>Venta neta</small><strong>$ ${money(m.salesNeto)}</strong><span>Facturas + Boletas ± NC/ND</span></div><div class="mcKpi mcAccentCyan"><small>Venta total</small><strong>$ ${money(m.salesTotal)}</strong><span>Neto + IVA después de ajustes</span></div><div class="mcKpi mcAccentGreen"><small>IVA</small><strong>$ ${money(m.salesIva)}</strong><span>Documentos comerciales</span></div><div class="mcKpi mcAccentAmber"><small>KG vendidos</small><strong>${qty(m.kilos)}</strong><span>Desde Maestro</span></div><div class="mcKpi mcAccentBlue"><small>Sacos</small><strong>${qty(m.sacos)}</strong><span>Valor Maestro</span></div><div class="mcKpi mcAccentCyan"><small>Granel despachado</small><strong>${qty(m.granelKg)} kg</strong><span>Desde despachos</span></div></div>
        <div class="mcV2Grid"><div class="mcV2Panel"><h4>Evolución de ventas netas</h4><div class="mcPanelSub">Últimos períodos disponibles en el Maestro</div>${barRows(latestMonths,m.maxMonthly,v=>'$ '+money(v))}<div class="mcMiniGrid"><div class="mcMini"><small>Período actual</small><b>$ ${money(recent)}</b></div><div class="mcMini"><small>Variación vs anterior</small><b>${delta===null?'—':(delta>=0?'+':'')+delta.toFixed(1)+'%'}</b></div></div></div><div class="mcV2Panel"><h4>Mix documental</h4><div class="mcPanelSub">Volumen de documentos del snapshot</div><div class="mcMix"><div class="mcDonut" style="background:conic-gradient(${mixStops.join(',')})"></div><div class="mcLegend">${m.docMix.map(([label,count])=>`<div class="mcLegendRow"><span>${label}</span><b>${count.toLocaleString('es-CL')}</b></div>`).join('')}</div></div></div></div>
        <div class="mcV2Grid"><div class="mcV2Panel"><h4>Top clientes por venta neta</h4><div class="mcPanelSub">Facturas + Boletas, con ajustes NC/ND</div>${barRows(m.topClients,Math.max(1,...m.topClients.map(([,v])=>Math.abs(v))),v=>'$ '+money(v))}</div><div class="mcV2Panel"><h4>Top productos por KG despachado</h4><div class="mcPanelSub">Consolidado desde el módulo de Despachos</div>${barRows(m.topProducts,Math.max(1,...m.topProducts.map(([,v])=>v)),v=>qty(v)+' kg')}</div></div>
        <div class="mcV2Grid"><div class="mcV2Panel"><h4>Control comercial</h4><div class="mcTopList"><div class="mcTopRow"><span>Facturas</span><b>${m.invoices.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>Boletas</span><b>${m.boletas.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>Notas de Crédito</span><b>${m.nc.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>Notas de Débito</span><b>${m.nd.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>Guías</span><b>${m.guides.toLocaleString('es-CL')}</b></div></div></div><div class="mcV2Panel"><h4>Control operativo</h4><div class="mcTopList"><div class="mcTopRow"><span>Despachos</span><b>${m.dispatches.length.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>KG despachados</span><b>${qty(m.dispatchKg)}</b></div><div class="mcTopRow"><span>Sacos despachados</span><b>${qty(m.dispatchSacos)}</b></div><div class="mcTopRow"><span>Clientes activos</span><b>${m.clients.length.toLocaleString('es-CL')}</b></div><div class="mcTopRow"><span>Productos activos</span><b>${m.products.length.toLocaleString('es-CL')}</b></div></div></div></div>
  ...
  function install(){
    ...
    if(typeof original==='function')window.show=function(page,...args){if(page==='macro'){renderMacro();return}return original.apply(this,[page,...args])};
    window.__LYRA_MACRO_V2__=Object.freeze({version:VERSION,render:renderMacro});
    window.__LYRA_MACRO_V1__=Object.freeze({version:VERSION,render:renderMacro});
