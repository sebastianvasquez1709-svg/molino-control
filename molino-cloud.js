...
(() => {
  ...
  async function signIn(identifier,password){
    ...
    localSession={local:true,user:{id:data.id,email:data.email,rut:data.rut,role:String(data.rol||'operador').toUpperCase(),nombre:data.nombre||data.email}};
  ...
  async function fetchSnapshot(){
    ...
    const sb=await client();
    const {data,error}=await withTimeout(sb.rpc('molino_app_snapshot_local',{p_rut:localSession._identifier,p_pin:localSession._password}));
    if(error) throw error;
  ...
  window.MolinoCloud=Object.freeze({config:Object.freeze({SUPABASE_URL}),client,getSession,signIn,signOut,health,snapshot,clearCache,cacheInfo,list});
})();