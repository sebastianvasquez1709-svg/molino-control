/* Molino Control · Panel Macro PRO V5
...
(() => {
  'use strict';
  if (window.__MC_MACRO_PRO_V5__) return;
  window.__MC_MACRO_PRO_V5__ = true;

  const STYLE_ID='mc-macro-pro-v5-style';
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .mcV5{--blue:#2b83ff;--green:#28c76f;--purple:#9b6cff;--orange:#f3a400;--cyan:#19c7da;--red:#ef5350;max-width:1700px;margin:0 auto;padding:16px;background:linear-gradient(145deg,#060708,#0c0f12 55%,#070809);border:1px solid #20262c;border-radius:18px;min-height:calc(100vh - 106px);box-shadow:0 16px 50px rgba(0,0,0,.28);font-family:Inter,Segoe UI,Arial,sans-serif}
      .mcV5 *{box-sizing:border-box}.mcV5Head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:12px}.mcV5Title h1{font-size:28px;letter-spacing:-.6px;margin:0;color:#fff}.mcV5Title p{margin:3px 0;color:#7d8791;font-size:10px}.mcV5Tools{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.mcV5Chip,.mcV5Btn{background:#101417;border:1px solid #2c333a;color:#dce2e7;border-radius:9px;padding:8px 10px;font-size:10px;font-weight:800}.mcV5Btn{cursor:pointer}.mcV5Status{display:inline-flex;align-items:center;gap:6px;border:1px solid #28543a;background:#0e1711;color:#a4e9bc;border-radius:999px;padding:7px 10px;font-size:10px;font-weight:800}.mcV5Status i{width:7px;height:7px;border-radius:50%;background:var(--green)}
      .mcV5Kpis{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:9px}.mcV5Kpi{background:linear-gradient(180deg,#121518,#0d1012);border:1px solid #282e34;border-radius:12px;min-height:112px;padding:12px;border-top:2px solid var(--accent)}.mcV5Kpi small{color:#8f98a1;font-size:8px;text-transform:uppercase;letter-spacing:.09em}.mcV5Kpi strong{display:block;color:#fff;font-size:21px;line-height:1.15;margin-top:7px}.mcV5Kpi span{display:block;color:#69747e;font-size:8px;margin-top:5px}
      .aBlue{--accent:var(--blue)}.aGreen{--accent:var(--green)}.aPurple{--accent:var(--purple)}.aOrange{--accent:var(--orange)}.aCyan{--accent:var(--cyan)}
      .mcV5Grid{display:grid;grid-template-columns:1.35fr .9fr 1fr;gap:9px;margin-top:9px}.mcV5Grid2{display:grid;grid-template-columns:1fr 1fr 1fr;gap:9px;margin-top:9px}.mcV5Panel{background:linear-gradient(180deg,#111417,#0d1012);border:1px solid #252c32;border-radius:12px;padding:13px;min-width:0}.mcV5Panel h3{margin:0;font-size:12px;color:#f6f7f8}.mcV5Sub{margin-top:3px;color:#747f89;font-size:8px}.mcV5Loading{display:grid;place-items:center;min-height:180px;color:#818c95;border:1px solid #2a3036;border-radius:10px;background:#0f1215}.mcV5Error{padding:20px;border:1px solid #5b3030;background:#1b0e0e;color:#ffb7b3;border-radius:10px}.mcV5Error strong{display:block;color:#ffd0cd;margin-bottom:5px}
      .mcV5Line svg{width:100%;height:220px;display:block;margin-top:8px}.mcGridLine{stroke:#252b31;stroke-width:1}.mcAxis{fill:#717c85;font-size:8px}.mcValue{fill:#e3e8ec;font-size:8px;font-weight:800}.mcPath{fill:none;stroke:var(--blue);stroke-width:3;stroke-linecap:round;stroke-linejoin:round}.mcPoint{fill:#0c1013;stroke:var(--blue);stroke-width:3}.mcArea{fill:rgba(43,131,255,.10)}
      ...
      @media(max-width:1250px){.mcV5Kpis{grid-template-columns:repeat(3,1fr)}.mcV5Grid{grid-template-columns:1.15fr .85fr}.mcV5Grid>.mcV5Panel:last-child{grid-column:1/-1}.mcV5Grid2{grid-template-columns:1fr 1fr}.mcV5Grid2>.mcV5Panel:last-child{grid-column:1/-1}}
      @media(max-width:760px){.mcV5{padding:11px}.mcV5Head{flex-direction:column}.mcV5Kpis{grid-template-columns:repeat(2,1fr)}.mcV5Grid,.mcV5Grid2{grid-template-columns:1fr}.mcV5Grid>.mcV5Panel:last-child,.mcV5Grid2>.mcV5Panel:last-child{grid-column:auto}.mcTiles{grid-template-columns:1fr 1fr}.mcBar{grid-template-columns:90px 1fr 74px}}
      @media(max-width:460px){.mcV5Kpis,.mcTiles{grid-template-columns:1fr}.mcV5Title h1{font-size:23px}}
    `;document.head.appendChild(s);
  }
  function monthlyChart(monthly){
    if(!monthly.length)return '<div class="mcV5Loading">No hay períodos de venta disponibles.</div>';
    ...
    return `<div class="mcV5Line"><svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><line class="mcGridLine" x1="${p}" y1="${h-p}" x2="${w-p}" y2="${h-p}"/><line class="mcGridLine" x1="${p}" y1="${h/2}" x2="${w-p}" y2="${h/2}"/><line class="mcGridLine" x1="${p}" y1="${p}" x2="${w-p}" y2="${p}"/><path d="${area}" class="mcArea"/><path d="${path}" class="mcPath"/>${pts.map(q=>`<circle class="mcPoint" cx="${q.x}" cy="${q.y}" r="4"/><text class="mcValue" x="${q.x}" y="${Math.max(11,q.y-8)}" text-anchor="middle">$${money(q.v/1000000)}M</text><text class="mcAxis" x="${q.x}" y="${h-6}" text-anchor="middle">${esc(q.k)}</text>`).join('')}</svg></div>`;
  }
  function bars(items,format,accent){if(!items.length)return '<div class="mcV5Loading">Sin datos para mostrar.</div>';const max=Math.max(1,...items.map(x=>Math.abs(n(x.value))));return '<div class="mcBars">'+items.map(x=>`<div class="mcBar"><label title="${esc(x.label)}">${esc(x.label)}</label><div class="mcTrack"><i style="--bar:${accent};width:${Math.max(3,Math.round(Math.abs(n(x.value))/max*100))}%"></i></div><b>${format(x.value)}</b></div>`).join('')+'</div>'}
  ...
  function errorView(msg){const c=$('content');if(!c)return;c.innerHTML=`<div class="mcV5"><div class="mcV5Head"><div><div class="mcV5Title"><h1>PANEL MACRO</h1><p>CENTRO DE CONTROL EJECUTIVO · MOLINO SAN MIGUEL LTDA.</p></div></div><button class="mcV5Btn" id="mcV5Retry">↻ Reintentar</button></div><div class="mcV5Error"><strong>No se pudo cargar el Centro de Control</strong>${esc(msg)}</div></div>`;$('mcV5Retry')?.addEventListener('click',render)}
  ...
  async function render(){
    ...
    c.innerHTML='<div class="mcV5"><div class="mcV5Head"><div><div class="mcV5Title"><h1>PANEL MACRO</h1><p>CENTRO DE CONTROL EJECUTIVO · MOLINO SAN MIGUEL LTDA.</p></div></div><div class="mcV5Tools"><span class="mcV5Status"><i></i> DATOS DE LA SESIÓN ACTIVA</span><span class="mcV5Chip">Analítica ejecutiva</span><button class="mcV5Btn" id="mcV5Refresh">↻ Actualizar</button></div></div><div class="mcV5Loading">Cargando datos del Maestro…</div></div>';
    try{
      ...
      c.innerHTML=`<div class="mcV5"><div class="mcV5Head"><div><div class="mcV5Title"><h1>PANEL MACRO</h1><p>CENTRO DE CONTROL EJECUTIVO · MOLINO SAN MIGUEL LTDA.</p></div></div><div class="mcV5Tools"><span class="mcV5Status"><i></i> MAESTRO CARGADO</span><span class="mcV5Chip">${esc(m.monthly.at(-1)?.periodo||'Período actual')}</span><button class="mcV5Btn" id="mcV5Refresh">↻ Actualizar</button></div></div>
      <div class="mcV5Kpis"><div class="mcV5Kpi aBlue"><small>Venta neta</small><strong>$ ${money(m.net)}</strong><span>Facturas + Boletas ± NC/ND</span></div><div class="mcV5Kpi aGreen"><small>Venta total</small><strong>$ ${money(m.total)}</strong><span>Neto + IVA ajustado</span></div><div class="mcV5Kpi aPurple"><small>IVA</small><strong>$ ${money(m.iva)}</strong><span>Documentos comerciales</span></div><div class="mcV5Kpi aOrange"><small>KG vendidos</small><strong>${qty(m.kg)}</strong><span>Facturas + Boletas</span></div><div class="mcV5Kpi aOrange"><small>Sacos vendidos</small><strong>${qty(m.sacos)}</strong><span>Maestro comercial</span></div><div class="mcV5Kpi aCyan"><small>Granel despachado</small><strong>${qty(m.granel)} kg</strong><span>${qty(m.despachos)} despachos</span></div></div>
      <div class="mcV5Grid"><section class="mcV5Panel"><h3>EVOLUCIÓN DE VENTAS NETAS</h3><div class="mcV5Sub">Últimos 12 períodos disponibles</div>${monthlyChart(m.monthly)}<div class="mcFooter"><span>Último: ${last===null?'—':'$ '+money(last)}</span><span>Variación: ${mom===null?'—':(mom>=0?'+':'')+mom.toFixed(1)+'%'}</span><span>Promedio: $ ${money(avg)}</span></div></section><section class="mcV5Panel"><h3>MIX DOCUMENTAL</h3><div class="mcV5Sub">Cantidad de documentos</div><div class="mcDocs">${docRows.map(([label,value,color])=>`<div class="mcDoc"><span>${label}</span><div class="mcDocTrack"><i style="--c:${color};width:${Math.max(2,Math.round(value/Math.max(1,m.counts.documents)*100))}%"></i></div><b>${value.toLocaleString('es-CL')}</b></div>`).join('')}</div><div class="mcFooter"><span>${m.counts.documents.toLocaleString('es-CL')} documentos</span><span>${m.counts.guias.toLocaleString('es-CL')} guías</span></div></section><section class="mcV5Panel"><h3>TOP 10 CLIENTES</h3><div class="mcV5Sub">Venta neta ajustada</div>${bars(m.topClients,x=>'$ '+money(x),'#2b83ff')}</section></div>
      <div class="mcV5Grid2"><section class="mcV5Panel"><h3>TOP 10 PRODUCTOS</h3><div class="mcV5Sub">KG vendidos</div>${bars(m.topProducts,x=>qty(x)+' kg','#28c76f')}</section><section class="mcV5Panel"><h3>CONTROL COMERCIAL</h3><div class="mcTiles"><div class="mcTile" style="--c:#2b83ff"><small>Facturas</small><b>${m.counts.facturas.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#28c76f"><small>Boletas</small><b>${m.counts.boletas.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#f3a400"><small>NC</small><b>${m.counts.nc.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#ef5350"><small>ND</small><b>${m.counts.nd.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#9b6cff"><small>Guías</small><b>${m.counts.guias.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#19c7da"><small>Precio neto/kg</small><b>$ ${money(m.precioKg)}</b></div></div></section><section class="mcV5Panel"><h3>CONTROL OPERATIVO</h3><div class="mcTiles"><div class="mcTile" style="--c:#2b83ff"><small>Despachos</small><b>${m.despachos.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#28c76f"><small>KG despachados</small><b>${qty(m.dkg)}</b></div><div class="mcTile" style="--c:#f3a400"><small>Sacos</small><b>${qty(m.sacos)}</b></div><div class="mcTile" style="--c:#9b6cff"><small>Clientes</small><b>${m.clientes.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#19c7da"><small>Productos</small><b>${m.productos.toLocaleString('es-CL')}</b></div><div class="mcTile" style="--c:#ef5350"><small>Granel</small><b>${qty(m.granel)} kg</b></div></div></section></div>
      <section class="mcV5Panel" style="margin-top:9px"><h3>AUDITORÍA DEL DATO</h3><div class="mcV5Sub">Controles preventivos del Centro de Control</div><div class="mcAudit"><div class="mcAuditRow ${m.qa.sin_fecha===0?'ok':'warn'}"><span>Documentos sin fecha</span><b>${m.qa.sin_fecha}</b></div><div class="mcAuditRow ${m.qa.comerciales_cero_neto===0?'ok':'warn'}"><span>Comerciales con neto $0</span><b>${m.qa.comerciales_cero_neto}</b></div><div class="mcAuditRow ${m.qa.facturas_negativas===0?'ok':'warn'}"><span>Facturas negativas</span><b>${m.qa.facturas_negativas}</b></div><div class="mcAuditRow ${m.qa.sin_cliente===0?'ok':'warn'}"><span>Comerciales sin cliente</span><b>${m.qa.sin_cliente}</b></div></div></section><div class="mcFooter"><span>Fuente: snapshot Maestro de la sesión activa · sin segunda autenticación.</span><span>${m.counts.documents.toLocaleString('es-CL')} documentos · ${m.despachos.toLocaleString('es-CL')} despachos</span></div></div>`;
      $('mcV5Refresh')?.addEventListener('click',render);
    }catch(e){errorView(e?.message||String(e))}
  }
  function install(){
    const original=window.show;
    if(typeof original==='function'&&!window.__MC_MACRO_V5_SHOW_WRAPPED__){window.__MC_MACRO_V5_SHOW_WRAPPED__=true;window.show=function(page,...args){const p=String(page||'').toLowerCase();if(p.includes('macro')||p.includes('control')){render();return}return original.apply(this,[page,...args])}}
    const target=$('content');if(target)new MutationObserver(()=>{const title=$('pageTitle');const active=title&&/panel macro|centro de control/i.test(title.textContent||'');document.documentElement.classList.toggle('mcMacroActive',!!active);if(active&&!document.querySelector('.mcV5'))render()}).observe(target,{childList:true,subtree:false});
